...
export default function PrompterPage() {
  ...
  useEffect(() => {
    ...
    const step = (timestamp: number) => {
      ...
      const reachedEnd = scrollRef.current.scrollTop + scrollRef.current.clientHeight >= scrollRef.current.scrollHeight - 8;
      if (reachedEnd) {
        setIsPlaying(false);
        return;
      }
    ...
    <div className="fixed inset-0 z-[80] flex flex-col bg-slate-950 text-white">
      <header className="flex flex-wrap items-center justify-between gap-3 border-b border-white/10 bg-slate-950/95 px-4 py-3 backdrop-blur sm:px-6">
        <div className="min-w-0">
          <p className="text-[11px] font-semibold uppercase tracking-[0.2em] text-amber-300">프롬프터 모드</p>
        ...
        <div className="flex flex-wrap items-center gap-2 text-xs sm:text-sm">
          <button
            ...
            className="inline-flex items-center gap-2 rounded-full bg-amber-300 px-4 py-2 font-bold text-slate-950 shadow-lg shadow-amber-300/20 disabled:opacity-40"
          >
      ...
      <div className="grid gap-3 border-b border-white/10 bg-slate-900/80 px-4 py-3 text-xs text-white/75 sm:grid-cols-2 sm:px-6">
        <label className="flex items-center gap-3">
          ...
          <input className="w-full accent-amber-300" type="range" min="12" max="120" step="2" value={scrollSpeed} onChange={(e) => setScrollSpeed(Number(e.target.value))} />
          <span className="w-16 text-right tabular-nums">{scrollSpeed}px/s</span>
        </label>
        <label className="flex items-center gap-3">
          <span className="w-20 font-semibold text-white">글자 크기</span>
          <input className="w-full accent-amber-300" type="range" min="28" max="72" step="2" value={fontSize} onChange={(e) => setFontSize(Number(e.target.value))} />
          <span className="w-12 text-right tabular-nums">{fontSize}px</span>
        </label>
      </div>

      <main ref={scrollRef} className="flex-1 overflow-y-auto scroll-smooth px-5 sm:px-10 lg:px-20">
        <div className="mx-auto max-w-5xl py-[45vh]">
          {sections.length === 0 ? (
            <div className="rounded-3xl border border-white/10 bg-white/5 p-8 text-center">
              ...
              <Link href="/scenario" className="mt-6 inline-flex rounded-full bg-amber-300 px-5 py-2 text-sm font-bold text-slate-950">시나리오로 돌아가기</Link>
            </div>
          ) : (
            <article className={mirrorMode ? "scale-x-[-1]" : ""} style={{ fontSize: `${fontSize}px` }}>
              {sections.map((section, index) => (
                <section key={`${section.title}-${index}`} className="mb-16 rounded-[32px] border border-white/10 bg-white/[0.04] p-8 shadow-[0_30px_80px_-50px_rgba(255,255,255,0.3)]">
                  <div className="mb-6 inline-flex rounded-full bg-amber-300/15 px-4 py-1 text-sm font-bold uppercase tracking-[0.2em] text-amber-200">{section.title}</div>
